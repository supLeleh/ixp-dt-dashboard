...
def _execute_rib_command(machine_name: str, lab) -> str:
    """
    Prova bgpctl (OpenBGPD), fallback automatico su birdc (BIRD2).
    RibDump gestisce entrambi i formati nativamente estraendo solo il prefisso IP.
    """
    try:
        result = execute_command_on_machine(machine_name, "bgpctl show rib", lab)
        logging.info(f"[RIB] Using OpenBGPD (bgpctl) on {machine_name}")
        return result if isinstance(result, str) else str(result)
    except Exception as e:
        if "bgpctl" not in str(e) and "Binary" not in str(e):
            raise

    logging.info(f"[RIB] bgpctl not found on {machine_name}, trying birdc (BIRD2)...")
    result = execute_command_on_machine(machine_name, "birdc show route all", lab)
    logging.info(f"[RIB] Using BIRD2 (birdc) on {machine_name}")
    return result if isinstance(result, str) else str(result)
...
async def get_ribs_diff(
    ...
    try:
        ...
        # Auto-detect: OpenBGPD o BIRD2
        logging.info(f"Executing RIB command on {machine_name} (auto-detect daemon)")
        actual_ribs_content = _execute_rib_command(machine_name, ServerContext.get_lab())
        ...
        logging.info(f"RIB diff: {len(intersection)} matching, {len(not_loaded)} not loaded, {len(extra_routes)} extra")
        ...
        result = {
            ...
            'not_loaded_routes': sorted([str(r) for r in not_loaded]),
            'extra_routes': sorted([str(r) for r in extra_routes]),
            'matching_routes': sorted([str(r) for r in intersection]),
        }
